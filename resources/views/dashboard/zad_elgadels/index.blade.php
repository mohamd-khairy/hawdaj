        ...
        <div class="card card-custom">
            ...
            <div class="card-body">
                ...
                <table class="table table-bordered dataTable2" id="">
                    ...
                    <tbody>
                        @if(!empty($zad_elgadels->first()))
                        @foreach($zad_elgadels as $index => $store)
                        <tr id="row-{{$store->id}}">
                            ...
                            <td>
                                ...
                                <a class="btn btn-sm btn-clean btn-icon btn-icon-md restore-button" title="{{__('dashboard.restore')}}" data-toggle="modal" data-target="#restore_modal" data-url="{{ route('dashboard.zad_elgadels.restore',$store->id) }}" data-item-id="{{ $store->id }}">
                                    <i class="flaticon2-refresh refresh-icon"></i>
                                </a>
                                <a class="btn btn-sm btn-clean btn-icon btn-icon-md delete-button" title="{{__('dashboard.force_delete')}}" data-toggle="modal" data-target="#delete_modal" data-url="{{ route('dashboard.zad_elgadels.destroy',[$store->id , 'archive' => request('archive')]) }}" data-item-id="{{ $store->id }}">
                                    <i class="flaticon2-trash trash-icon"></i>
                                </a>
                                @else

                                <a href="{{route('dashboard.zad_elgadels.edit', $store->id)}}" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="{{__('dashboard.edit')}}">
                                    <i class="flaticon-edit-1 edit-icon"></i>
                                </a>
                                <a class="btn btn-sm btn-clean btn-icon btn-icon-md delete-button" title="{{__('dashboard.delete')}}" data-toggle="modal" data-target="#delete_modal" data-url="{{ route('dashboard.zad_elgadels.destroy',$store->id) }}" data-item-id="{{ $store->id }}">
                                    <i class="flaticon2-trash trash-icon"></i>
                                </a>
                                @endif
                            </td>
