@if(!empty($completed_requests->first()))
    <table class="table table-bordered" id="dataTable2">
        ...
        <tbody>
        @foreach($completed_requests as $index => $request)
            <tr id="row-{{$request->id}}">
                ...
                <td>
                    @if($request->type == 'between_sites')
                        <small>{{__('dashboard.site')}}</small>: {{ $request->sender_site->name}}<br>
                        <small>{{__('dashboard.employee')}}</small>: {{ $request->sender_host->full_name}}
