...
{{--                        {{ dd(\Str::snake($details['bar']['title']) ) }}--}}
                        @php($title = \Str::snake($details['bar']['title']))
                        {{ $title ? handleTrans($title,$details['bar']['title']) : __('dashboard.bar_chart') }}
                         <i class="ml-1 fas fa-info-circle pointer" style="color: #666"
                            data-toggle="tooltip" data-placement="top"
                            title="{{$details['bar']['description'] ?? __('dashboard.bar_chart') }}"
                         ></i>
                    </span>
                </h3>
                @if($add_pinned ??false == true)
                    <div>
                        <button class="btn btn-sm btn-outline-success add_pinned"
                                data-chart_id="{{$details['bar']['id']}}">
        ...
        <div class="card-body">
            <div @isset($chart_key) id="bar_{{$chart_key}}" @else id="chart1" @endif></div>
        </div>
    </div>
</div>
